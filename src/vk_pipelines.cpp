...
bool vkutil::load_shader_module(
  ...
  // spirv expects the buffer to be on uint32, so make sure to serve a int vector big enough for the entire file
  ...
  // codeSize has to be in bytes, so multiply the ints in the buffer by size of int to know th ereal size of the buffer
